...
func (pc *PerformanceCollector) Collect(ctx *MetricsContext) map[string]interface{} {
	...
	// MCP loop health metrics (when MCP mode is used)
	if ctx.Config != nil {
		metrics["mcp_tool_calls_total"] = ctx.Config.LastMCPToolCallsTotal
		metrics["mcp_unique_tool_calls"] = ctx.Config.LastMCPUniqueToolCalls
		metrics["mcp_repeated_tool_calls"] = ctx.Config.LastMCPRepeatedToolCalls
		metrics["mcp_cache_hits"] = ctx.Config.LastMCPCacheHits
		metrics["mcp_stop_reason"] = strings.TrimSpace(ctx.Config.LastMCPStopReason)
	}
